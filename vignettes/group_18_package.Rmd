...
## `translate`
This function takes as argument a vector of strings (codons), converts each codon to its corresponding amino acid (1-letter code) based on the codon table, and concatenates the letters into a single string (peptide).
```{r}
translate("AUG")
```
...
5. The occurrences of each amino acid in the peptide is counted by `count_aa` and displayed in a bar plot.
